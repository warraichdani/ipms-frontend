            ...
            <ModalBody>
                <Formik
                    ...
                    onSubmit={async (values, { setSubmitting }) => {
                        ...
                        if (!inv) {
                            toast.error("Investment is required");
                            setSubmitting(false);
                            return;
                        }
                        try {
                    ...
                    {({ values, isSubmitting, setFieldValue }) => {
                        const newValue =
                            values.transactionType === "Buy"
                                ? (values.investment?.currentValue ?? 0) + Number(values.amount || 0)
                                : values.transactionType === "Sell"
                                    ? Math.max(0, (values.investment?.currentValue ?? 0) - Number(values.amount || 0))
                                    : values.transactionType === "UpdatePrice"
                                        ? Number(values.amount || (values.investment?.currentValue ?? 0))
                                        : values.investment?.currentValue ?? 0;
                        return (
                            <Form className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        {investment ? (
                                            <Select disabled value={values.investmentId}>
                                                <option value={values.investmentId}>{investment.investmentName}</option>
                                            </Select>
                                        ) : (
                                            <InvestmentDropdown
                                                selectedId={values.investmentId}
                                                onChange={(id) => setFieldValue("investmentId", id)}
                                                onSelectInvestment={(inv) => setFieldValue("investment", inv)}
                                            />
                                        )}
